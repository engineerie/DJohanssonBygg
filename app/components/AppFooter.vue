...
<template>
  <UFooter class="z-10 bg-default border-t border-primary-500/30"
    :ui="{ left: 'text-muted text-xs', container: 'sm:border-x border-primary-500/30' }">
    <template #left>
      {{ footer.credits }}
    </template>
    ...
    <template #right>
      <div class="space-x-6 text-center sm:flex">
        ...
        <!-- <div class="flex gap-2 flex-nowrap">
          <UIcon :name="footer.location.icon" class="text-daniel-500 mt-0.5" />
          <span>{{ footer.location.street }}, {{ footer.location.postCode }} {{ footer.location.county }}</span>
        </div> -->
      </div>
